...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, sky_model: SkyModel=None, load_iteration=None, shuffle=False, resolution_scales=[1.0], only_pose=False, splatting_ply_path=None):
        ...
        if not self.loaded_iter:
            # Copy COLMAP point cloud to model directory
        ...
        for resolution_scale in resolution_scales:
            for frame_id in range(len(scene_info.train_cameras)):
                ...
                prospective2orthogral = getProjectionMatrix(
                    znear=0.01,
                    zfar=100.0,
                    fovX=fovx,
                    fovY=fovy,
                ...
                full_proj_matrix = prospective2orthogral @ w2c
    ...
    def save(self, iteration):
        ...
        self.gaussians.save_ply(os.path.join(point_cloud_path, "colmap_point_cloud.ply"))
        self.gaussians.save_semantic_ply(os.path.join(point_cloud_path, "colmap_semantic_point_cloud.ply"))
        self.gaussians.save_opacity_ply(os.path.join(point_cloud_path, "colmap_opacity_point_cloud.ply"))
    ...
    def getPcdInTrainFrame(self, pcd, frame_id, scale=1.0):
        colmap_pcd = pcd
        w2c = self.w2c_list[scale][frame_id]
        camera_point = (w2c[:3, :3] @ colmap_pcd.T).T + w2c[:3, 3]
