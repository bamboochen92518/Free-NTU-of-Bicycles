...
def loadCam(args, id, cam_info, resolution_scale, only_pose=False):
    # Try to get image size from cam_info.image
    try:
        if cam_info.image is not None:
            orig_w, orig_h = cam_info.image.size
        else:
            # Fallback: Use image dimensions from COLMAP or args
            orig_w, orig_h = cam_info.width, cam_info.height
            if orig_w is None or orig_h is None:
                raise ValueError(f"cam_info.image is None and width/height not provided for camera {cam_info.image_name}")
    except AttributeError as e:
        print(f"Error accessing cam_info.image.size for {cam_info.image_name}: {e}")
        # Fallback to COLMAP intrinsics or args
        orig_w, orig_h = cam_info.width, cam_info.height
        if orig_w is None or orig_h is None:
            raise ValueError(f"Cannot determine image size for {cam_info.image_name}")
    ...
    if only_pose:
        ...
        if cam_info.image is None:
            print(f"Warning: cam_info.image is None for {cam_info.image_name}, using empty tensor")
            resized_image_rgb = torch.empty((3, resolution[1], resolution[0]), device=args.data_device)
        else:
