...
def training_report(tb_writer, iteration, loss_dict, loss, elapsed, testing_iterations, scene: Scene, renderFunc, renderArgs, sky_model):
    ...
    metrics_file = os.path.join(args.model_path, "training_metrics.json")
    metrics_data = []
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = (
            {'name': 'test', 'cameras': [] if len(scene.getTestCameras()) == 0 else [scene.getTestCameras()[idx % len(scene.getTestCameras())] for idx in pick_frame_list]},
            {'name': 'train', 'cameras': [scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in pick_frame_list]}
        )

        # Prepare metrics for this iteration
        iteration_metrics = {
            "iteration": iteration,
            "train": {"l1_loss": 0.0, "psnr": 0.0},
            "test": {"l1_loss": 0.0, "psnr": 0.0},
            "losses": {key: value.item() for key, value in loss_dict.items()},
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if tb_writer and (idx < 8):
                        ...
                        tb_writer.add_images(config['name'] + "_view_{}/rend_semantic".format(viewpoint.image_name), semantic_pkg['semantic_rgb'][None], global_step=iteration)
                ...
                iteration_metrics[config['name']]['l1_loss'] = float(l1_test)
                iteration_metrics[config['name']]['psnr'] = float(psnr_test)
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {}".format(iteration, config['name'], l1_test, psnr_test))
                if tb_writer:
                    tb_writer.add_scalar(config['name'] + '/loss_viewpoint - l1_loss', l1_test, iteration)
                    tb_writer.add_scalar(config['name'] + '/loss_viewpoint - psnr', psnr_test, iteration)

        # Append or update metrics data
        metrics_data.append(iteration_metrics)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--continue_model_path", type=str, default=None)
